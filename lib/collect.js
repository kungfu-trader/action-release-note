...
exports.createRecords = async function (argv, pulls) {
  ...
  return await createAirtableRecord.submit(argv.baseId, argv.tableId);
};
...
class CreateAirtableRecord {
  ...
  async submit(baseId, tableId) {
    ...
    const insertRecords = this.records.filter(
      (v) => !origin.find((x) => +x.number === +v.fields.number && +x.body === +v.fields.body)
    );
    ...
    const deleteRecords = origin
      .filter((v) => !this.records.find((x) => +x.fields.number === v.number && +x.fields.body === +v.body))
      .map((v) => v.id);
