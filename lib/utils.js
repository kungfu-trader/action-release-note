...
const markDownToPdf = async (fileName, content) => {
  const converter = new MarkdownToPDF();
  const pdf = await converter.convert(content);
  if (pdf) {
    fs.writeFileSync(fileName.replace(".md", ".pdf"), pdf.pdf);
...
function teleport(argv, version) {
  ...
  const sources = ["notes", "pdfs"].map((v) => path.join(process.cwd(), v));
  getArtifactNameMap().forEach((name) => {
